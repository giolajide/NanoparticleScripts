...
This script tracks heights by what layer each atom belongs to
    ...
    In this version, for height-varying strain, we consider intra- 
    and inter-layer bonds,
...
def compute_strain(
    ...
    return np.array(strains), np.array(surface_strains), np.array(layer_percents)
...
def write_output(
    ...
    surf_strain_layers = {f"Surface {i}": [] for i in strain_layers.keys()}
    ...
    for atoms in tqdm(traj,
        ...
        strain, surf_strain, layer_percent = compute_strain(
                atoms,
                nl = nl,
                lattice_constant = a,
        ...
        ##someday I'll make the below more concise; both strains and surface-strains
        #can be collected at once

        ##collect layer-varying-averaged surface strains
        for key in surf_strain_layers.keys():
            split_key = key.split()
            low = split_key[1]
            low = float(low.split("%")[0])
            high = split_key[-1]
            ...
            qualifying_surf_strains = np.array(surf_strain[qualifying_mask])
            surf_strain_layers[key].append(
                    np.nanmean(qualifying_surf_strains) if qualifying_surf_strains.size else np.nan
                    )


        ##collect layer-varying-averaged strains
    ...
    #height-varying strains and surface strains
    for key in strain_layers.keys():
        df.loc[:, key] = strain_layers[key]
    for key in surf_strain_layers.keys():
        df.loc[:, key] = surf_strain_layers[key]
